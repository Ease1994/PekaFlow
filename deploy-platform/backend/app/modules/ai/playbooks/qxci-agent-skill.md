...
- 流水线里多一个步骤、写 `task.py` → `qxci-plugin-draft`
- 当前就是在问发布状态，并没有要加技能 → `qxci-observe`
- 库里已有的技能要安装 / 卸载 / 停用 / 启用 → `qxci-skill-lifecycle`
- 助手要调一个平台还没有的 API → 第三方工具包，不是技能包

## Constraints

- 禁止 `qxci_atom_sdk`、`task.py`、`task.json`。
- 不要发明工具名。只能教模型调用已有工具。
- 不要占用 `qxci-*` 内置名。
