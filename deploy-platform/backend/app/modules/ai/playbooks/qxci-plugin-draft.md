...
2. Python 用 `import qxci_atom_sdk as sdk`：`sdk.get_input()`、`sdk.log.info()`、`sdk.set_output()`。回调平台用 `sdk.get_task_token()`，不要读 `QXCI_AGENT_TOKEN`。
...
- 给助手写 SKILL.md、实现 agent 技能 → `qxci-agent-skill`
- 当前就要发布或查状态 → `qxci-release` / `qxci-observe`
