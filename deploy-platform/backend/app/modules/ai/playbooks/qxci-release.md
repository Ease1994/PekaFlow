...
4. 没有执行权限时把错误原文转达，再加载 `qxci-access`（`apply_pipeline_execute`）。不要一上来把发布理解成申请权限。
5. 确认卡就是确认。回「确认 / 好的 / 是 / 提交」时完成未完成的卡片。不要说「已经发布成功」。
6. 回复只写真正传给工具的参数。

## Do not use

- 申请权限、没有权限、要执行权 → `qxci-access`
- 把上传的文件拷到某台机器 → `qxci-node-push`
- 只要流水线目录 → 一次 `list_pipelines`
- 停掉正在跑的、回滚已成功的、按原 commit 重跑 → `qxci-release-control`
- 问发得怎样了、失败原因、日志 → `qxci-observe` / `qxci-diagnose`
