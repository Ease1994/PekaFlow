...
4. 只要现场日志原文 → `get_release_logs`。分析原因改加载 `qxci-diagnose`。
5. `status=all` 不是把历史全拉出来。

## Do not use

- 要去发布 / 部署 / 上线 → `qxci-release`
- 为什么失败、帮我诊断 → `qxci-diagnose`
- 本周计划、卡在谁、同步给业务 → `qxci-pm`
- 给助手加一个「查状态」技能 → `qxci-agent-skill`（当前这一句如果只是在问状态，直接查，不要起草）
