...
def _validate(title: str, manifest: str) -> None:
    if not (title or "").strip():
        raise BizException.bad_request("请填写发布标题")
    if not (manifest or "").strip():
        raise BizException.bad_request("请填写发布清单（要发布哪些文件）")
...
def update_request(db: Session, request_id: int, body: dict) -> DeployRequest:
    ...
    if req.status == "submitted":
        _validate(req.title, req.manifest)
    db.commit()
    db.refresh(req)
    return req
