...
开发人员提单（要发哪些文件、改了什么），发布人员核对后一键触发流水线。
单子不参与调度，触发走的还是现有的 create_release，所以对发布流程零侵入。
"""
...
def _assert_manifest_pipeline(db: Session, pipeline_id) -> None:
    """挡住「选了一条根本不读清单的流水线」。

    不挡的话清单会照样写进 run_params 然后无人问津，发出去的东西和单子上写的
    对不上，而且全程没有任何报错——这种静默失效最难查。
    """
    if not pipeline_id:
        return
    ...
    if not service.consumes_deploy_manifest(p):
        raise BizException.bad_request(
            f"流水线「{p.name}」不是按清单增量发布的（编排里没有 pack-incremental，"
            "也没有用到 ${{DEPLOY_MANIFEST}}），发布清单交给它不会生效。"
            "发布提交单只适用于 Windows/IIS 这类按清单挑文件增量发布的流水线。"
        )
...
def create_deploy_request(
    ...
    require_project_visible(db, current, project_id)
    _assert_manifest_pipeline(db, body.get("pipeline_id"))
