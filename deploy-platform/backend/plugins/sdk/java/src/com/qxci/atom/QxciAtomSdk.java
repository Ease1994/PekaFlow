...
/**
 * qxci Atom SDK（JDK 8，无第三方依赖）。
 * Agent 通过环境变量注入上下文；插件写 stdout 日志与 .qxci_atom_output.json。
 */
public final class QxciAtomSdk {
    private QxciAtomSdk() {}
    ...
    public static void setOutput(String status, String message, Map<String, String> data) {
        ...
        json.append("}}");
        writeFile(new File(getWorkspace(), ".qxci_atom_output.json"), json.toString());
        writeFile(new File(System.getProperty("user.dir"), ".qxci_atom_output.json"), json.toString());
    }
