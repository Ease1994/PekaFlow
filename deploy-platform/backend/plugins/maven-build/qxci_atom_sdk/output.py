# -*- coding: utf-8 -*-
"""插件输出：写入工作空间 .qxci_atom_output.json，供 Agent 解析（如 source_ref）。"""
...
def set_output(output: dict[str, Any]) -> None:
    ...
    """
    ws = get_workspace()
    path = os.path.join(ws, ".qxci_atom_output.json")
    # 也写插件 cwd，兼容 Agent 在插件目录启动的场景
    cwd_path = os.path.join(os.getcwd(), ".qxci_atom_output.json")
