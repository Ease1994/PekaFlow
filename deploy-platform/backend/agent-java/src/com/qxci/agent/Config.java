...
public class Config {
    ...
    /**
     * 节点备份根。安装脚本按执行安装命令的目录第一层建好（/data/soft/qxci → /data/qxci-backup）。
     * 必须至少两层（不能是 / 或 /qxci-backup），流水线 backupDir 留空时用这里。空则 Linux 退到 /var/qxci/backup。
     */
    ...
    public static Config parse(String[] args) {
        ...
        if (c.enrollToken.isEmpty()) {
            String env = System.getenv("QXCI_ENROLL_TOKEN");
            if (env != null) {
                c.enrollToken = env.trim();
            }
        }
        if (c.home.isEmpty()) {
            String env = System.getenv("QXCI_AGENT_HOME");
            c.home = (env != null && !env.trim().isEmpty())
                    ? env.trim()
                    : new File(System.getProperty("user.home", "."), ".qx-agent").getAbsolutePath();
        }
    ...
    /**
     * 旧版本把凭据固定放在 ~/.qx-agent 下。守护化之后家目录可能被显式指到
     * ProgramData 之类的公共位置，这里兜底读一次旧路径，避免已装好的机器升级后
     * 要重新拿接入凭证。
     */
    private File legacyCredentialFile() {
        File legacyHome = new File(System.getProperty("user.home", "."), ".qx-agent");
