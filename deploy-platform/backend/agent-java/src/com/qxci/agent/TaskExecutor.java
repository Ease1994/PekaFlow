...
public class TaskExecutor implements StepRunner {
    ...
    private boolean runAtomPlugin(Map<String, Object> pkg, Map<String, Object> with,
        ...
        File pluginDir = new File(new File(new File(home, ".qx-agent/plugins"), name), version);
        File marker = new File(pluginDir, ".qxci_ready");
        File zip = new File(new File(home, ".qx-agent/plugin-cache"), name + "-" + version + ".zip");
        ...
        try {
            ...
            env.put("QXCI_WORKSPACE", pipelineDir.getAbsolutePath());
            env.put("QXCI_SRC", srcDir.getAbsolutePath());
            env.put("BK_CI_WORKSPACE", pipelineDir.getAbsolutePath());
            env.put("QXCI_ATOM_INPUT_JSON", Json.stringify(with));
            env.put("QXCI_PIPELINE_ID", String.valueOf(pipelineId));
            env.put("QXCI_BUILD_ID", String.valueOf(taskId));
            env.put("QXCI_RELEASE_ID", String.valueOf(releaseId));
            env.put("QXCI_JOB_NAME", jobName == null ? "" : jobName);
            env.put("QXCI_SERVER_URL", api.serverUrl());
            // 只给插件本次任务的凭证。Agent 自己的长期 token 一旦泄进插件进程，
            // 插件就能领走别人的任务、伪造任务状态，所以这里不注入。
            env.put("QXCI_TASK_TOKEN", taskToken == null ? "" : taskToken);
            env.put("QXCI_AGENT_ID", String.valueOf(api.agentId()));
    ...
    private void pickSourceRefFromOutput(File pipelineDir, File pluginDir, LogSink sink) {
        File[] files = new File[] {
                new File(pipelineDir, ".qxci_atom_output.json"),
                new File(pluginDir, ".qxci_atom_output.json"),
        };
        for (File f : files) {
            if (!f.isFile()) {
                continue;
