...
def test_jar_download_rejects_staff_when_agent_menus_admin_only(monkeypatch) -> None:
    """构建机和节点菜单都改成仅管理员后，普通人 JWT 不能再拉 jar。"""
    db = _db()
    db.add(PlatformSetting(key="menu_audience", value='{"agents":"admin","nodes":"admin"}'))
    db.commit()
    staff = CurrentUser(id=7, username="dev", is_admin=False)
    monkeypatch.setattr("app.core.deps.verify_token", lambda t, *a, **k: staff if t == "staff-jwt" else None)
    try:
